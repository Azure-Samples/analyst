...
class StatisticalAnalysisPlugin:
    ...
    @kernel_function(
        ...
        description="Calculate a comprehensive set of statistics for a 1D numeric dataset."
    )
    ...
    @kernel_function(
        ...
        description="Perform the Kolmogorov-Smirnov test for goodness-of-fit."
    )
    ...
    @kernel_function(
        ...
        description="Perform an independent two-sample t-test."
    )
    ...
    @kernel_function(
        name="FTest",
        description="Perform an F-test for equality of variances between two samples."
    )
    def f_test(self, sample1, sample2):
        ...
        cdf_value = float(stats.f.cdf(f_stat, df1, df2))
        p_value = 2 * min(cdf_value, 1 - cdf_value)
        return f_stat, p_value

    @kernel_function(
        name="ChiSquareTest",
        description="Perform a chi-square goodness-of-fit test."
    )
    ...
    @kernel_function(
        name="ComputeCategoryStatistics",
        description="Compute a matrix of descriptive statistics for each category in a dataset."
    )
