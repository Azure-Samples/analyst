...
from app.plugins import StatisticalAnalysisPlugin, SeabornPlugin, CodeRunnerPlugin
...
class Statistian(ToolerBase):
    ...
    async def interact(self, prompt: str, chat: ChatHistory) -> str:  # pylint: disable=arguments-differ
        ...
        :return: The aggregated response generated by the agent.
        """
        await self.add_tools([StatisticalAnalysisPlugin()])
    ...
    async def _llm_processing(self, prompt: str) -> List:
        ...
        kernel.add_plugins([
            KernelPlugin(
                ...
                description=grader.tooler.objective,
                functions=[grader.interact],
            ) for grader in self.graders
        ])
        instruction_template = JINJA_ENV.get_template("reason.jinja")
        rendered_settings = instruction_template.render(tools=[grader.tooler.name for grader in self.graders])
